...
(function(){
  ...
  const ITEMS=[
    ...
    {id:'sofa',type:'диван модульный 2 м, низкий, без подлокотников (основные модули)',room:4,layer:'kitchen',pos:[11.35,6.287-0.9],rot:0,size:[2.0,0.85,0.88],glb:'models/sofa.glb',coat:{upholstery:'sofaWeave',piping:'sofaWeave',cushion:'sofaWeave'}, // model by tools/models/sofa.js
     build(b){ b.phys(0,2,0.1,0.85,0,0.88); b(0,2,0.05,0.42,0,0.88,mat.sofa); [0,1.0].forEach(x=>b(x,x+1.0,0.42,0.72,0.60,0.88,mat.sofa)); [0.2,1.1].forEach(x=>b(x,x+0.65,0.42,0.83,0.50,0.62,mat.cushion)); }}, // fallback: seat block, two back blocks, pillows
    {id:'sofa2',type:'угловой модуль дивана (шезлонг) 0.90×0.80 у восточной стены, продолжает сиденье к ТВ',room:4,layer:'kitchen',pos:[12.45,6.287-0.9-0.8],rot:0,size:[0.90,0.85,0.80],glb:'models/chaise.glb',coat:{upholstery:'sofaWeave',piping:'sofaWeave',cushion:'sofaWeave'}, // model by tools/models/chaise.js
     build(b){ b.phys(0,0.9,0.1,0.85,0,0.8); b(0,0.9,0.05,0.42,0,0.8,mat.sofa); b(0.7,0.9,0.42,0.64,0,0.8,mat.sofa); b(0.55,0.68,0.42,0.83,0.1,0.65,mat.cushion); }}, // fallback: seat, arm block on the east, pillow
