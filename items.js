...
(function(){
  ...
  const SLOTS={chrome:['handle','knob','ring'],metal:['frame','kleg'],glass:['glass','rail'],fabric:['sofa','cushion','pouf','pillow','kmat','fabric','rug','tulle','drape'],emitter:['led'],screen:['screen'],wood:['table'],paint:['chair','base','upper','door','body','wbody','wdoor','wpanel','kbody'],plastic:['plastic'],ceramic:['ceramic'],acrylic:['acrylic'],leather:['leather'],mirror:['mirror']};
  ...
  const ITEMS=[
    ...
    {id:'sw2',type:'выключатель у двери, 2 клавиши: общий свет M11 + LED M12',room:2,layer:'kid2',pos:[11.38,6.538],rot:0,size:[0.08,0.99,0.01],fixed:'wall',build(b){ b.plate(0,0.08,0.91,0.99,0,0.01,mat.plastic,{keys:2}); }},
    // sockets: flat boxes 0.08 × 0.08 × 0.01 on the wall, purpose in the caption
    {id:'sock9',type:'розетки 2+2 USB над столом',room:2,layer:'kid2',pos:[11.087,8.57],rot:0,size:[0.01,0.94,0.08],fixed:'wall',build(b){ b.plate(0,0.01,0.86,0.94,0,0.08,mat.plastic,{keys:2}); }},
    {id:'sock10',type:'блок ПК под столом: 3 розетки + RJ-45',room:2,layer:'kid2',pos:[11.087,9.22],rot:0,size:[0.01,0.34,0.08],fixed:'wall',build(b){ b.plate(0,0.01,0.26,0.34,0,0.08,mat.plastic); }},
    {id:'sock11',type:'розетка у изголовья (ночник)',room:2,layer:'kid2',pos:[11.38,9.584],rot:0,size:[0.08,2.09,0.01],fixed:'wall',build(b){ b.plate(0,0.08,2.01,2.09,0,0.01,mat.plastic); }},
    {id:'sock12',type:'розетка у лежанки (зарядка), в нише',room:2,layer:'kid2',pos:[13.83,9.489],rot:0,size:[0.08,0.34,0.01],fixed:'wall',build(b){ b.plate(0,0.08,0.26,0.34,0,0.01,mat.plastic); }},
    ...
    {id:'mcab',type:'блок подвесных ящиков над изголовьем: 2 дверцы + секция под кондиционер',room:3,layer:'master',pos:[14.76,13.144],rot:180,size:[1.70,2.70,0.35],fixed:'wall',
     build(b,g){ b.phys(0,1.7,2.68,2.7,0,0.35); b.phys(0.9,1.7,1.85,1.87,0,0.35); b.phys(0.9,1.7,1.87,2.68,0,0.02); [0,0.9,1.3,1.68].forEach(x=>b.phys(x,x+0.02,1.87,2.68,0.02,0.35)); [0.923,1.323].forEach(x=>{ b.phys(x,x+0.354,1.873,2.677,0.33,0.35); b.phys(x+0.15,x+0.2,2.1,2.12,0.35,0.365); }); for(let y=1.89;y<2.66;y+=0.04) b.phys(0.02,0.88,y,y+0.02,0.33,0.35); // proxy = pre-detail mesh AABBs (realism-all D1)
       const t=0.02;
       b(0,1.7,2.7-t,2.7,0,0.35,mat.body); b(0.9,1.7,1.85,1.85+t,0,0.35,mat.body); b(0.9,1.7,1.85+t,2.7-t,0,t,mat.body); [0,0.9,1.3,1.68].forEach(x=>b(x,x+t,1.85+t,2.7-t,t,0.35,mat.body)); // top, bottom, back and section walls; AC section open below
       [0.923,1.323].forEach(x0=>{ b.round(x0,x0+0.354,1.873,2.677,0.33,0.35,0.001,mat.wdoor); b(x0+0.127,x0+0.227,1.873,1.893,0.347,0.3505,mat.frame); }); // doors with 3 mm gaps, flush pull profiles along the bottom edge
       for(let y=1.89;y<2.66;y+=0.04) g.add(new THREE.Mesh(new THREE.BoxGeometry(0.86,0.012,0.024).rotateX(-35*Math.PI/180).translate(0.45,y+0.01,0.336),mat.wdoor)); }},
    {id:'mward',type:'шкаф на две двери, вплотную к кровати; ниша-полка со стороны кровати 0.45–0.80',room:3,layer:'master',pos:[13.06,13.144],rot:180,size:[1.00,2.70,0.60],fixed:'wall',
     build(b){ b.phys(0,0.02,0,0.45,0.03,0.58); b.phys(0,0.02,0.8,2.7,0.03,0.58); b.phys(0.98,1,0,2.7,0.03,0.58); b.phys(0,1,2.68,2.7,0.03,0.58); b.phys(0,1,0,0.02,0.03,0.58); b.phys(0.02,0.98,0.02,2.68,0.03,0.05); b.phys(0,0.3,0.45,0.47,0.03,0.58); b.phys(0,0.3,0.78,0.8,0.03,0.58); b.phys(0.28,0.3,0.45,0.8,0.03,0.58); b.phys(0.02,0.496,0.02,2.68,0.58,0.6); b.phys(0.504,0.98,0.02,2.68,0.58,0.6); b.phys(0.46,0.475,1,1.3,0.6,0.62); b.phys(0.525,0.54,1,1.3,0.6,0.62); // proxy = pre-detail mesh AABBs (realism-all D1)
       ...
       b.round(0.02,0.4985,0.02,2.68,0.58,0.60,0.001,mat.wdoor); b.round(0.5015,0.98,0.02,2.68,0.58,0.60,0.001,mat.wdoor); // two doors, 3 mm gap
       b(0.4775,0.4975,1.0,1.3,0.5975,0.6005,mat.frame); b(0.5025,0.5225,1.0,1.3,0.5975,0.6005,mat.frame); }},
    ...
    {id:'tubmixer',type:'смеситель ванны настенный, излив 0.20',room:9,layer:'bath',pos:[8.192,9.124],rot:0,size:[0.20,0.86,0.20],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.05,0.75,0.85,0.02,0.18); b.phys(0.05,0.2,0.78,0.8,0.09,0.11); b.phys(0.05,0.1,0.85,0.86,0.09,0.11); b(0,0.05,0.75,0.85,0.02,0.18,mat.lamp); b(0.05,0.20,0.78,0.80,0.09,0.11,mat.lamp); b(0.05,0.10,0.85,0.86,0.09,0.11,mat.handle); }}, // body, spout, lever
    ...
    {id:'basindrawer',type:'ящик под раковиной 0.85×0.31×0.18, подвесной, push-to-open',room:9,layer:'bath',pos:[8.90,8.181],rot:0,size:[0.85,0.68,0.31],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.85,0.5,0.68,0,0.29); b.phys(0.005,0.845,0.505,0.675,0.29,0.31); b(0,0.85,0.50,0.68,0,0.29,mat.body); b(0.005,0.845,0.505,0.675,0.29,0.31,mat.wdoor); }},            // body, front to the south
    ...
    {id:'basinmixer',type:'смеситель раковины настенный, излив 0.18; ось чаши x 9.325',room:9,layer:'bath',pos:[9.275,8.185],rot:0,size:[0.14,1.09,0.20],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.1,1,1.09,0,0.02); b.phys(0.04,0.06,1.035,1.055,0.02,0.18); b.phys(0.1,0.14,1.04,1.05,0,0.02); b(0,0.10,1.00,1.09,0,0.02,mat.lamp); b(0.04,0.06,1.035,1.055,0.02,0.18,mat.lamp); b(0.10,0.14,1.04,1.05,0,0.02,mat.handle); }}, // body, spout, lever
    ...
    {id:'drain8',type:'линейный трап 1.40×0.06 вдоль западной стены',room:8,layer:'bath2',pos:[8.25,11.65],rot:0,size:[0.06,0.012,1.40],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.06,0.01,0.012,0,1.4); b(0,0.06,0.010,0.012,0,1.40,mat.handle); }},
    {id:'curb8e',type:'бортик душа 0.05×0.05 по восточной кромке на всю длину; вход в душ с юга (z 12.45–13.12) переступается',room:8,layer:'bath2',pos:[9.048,11.588],rot:0,size:[0.05,0.05,1.536],
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.05,0,0.05,0,1.536); b(0,0.05,0,0.05,0,1.536,mat.kmat); }},
    {id:'glass8',type:'неподвижное стекло душа 0.86×2.05 на бортике со стороны унитаза (z 11.59–12.45)',room:8,layer:'bath2',pos:[9.088,11.588],rot:0,size:[0.01,2.10,0.862],
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.01,0.05,0.07,0,0.862); b.phys(0,0.01,0.07,2.1,0,0.862); b(0,0.01,0.05,0.07,0,0.862,mat.frame); b(0,0.01,0.07,2.10,0,0.862,mat.glass); }},                     // profile, glass
    ...
    {id:'mixer8',type:'термостат душа h 1.10 и ручная лейка h 1.60 в накладном коробе 0.08 (южная стена наружная); восточнее полки',room:8,layer:'bath2',pos:[8.90,13.044],rot:0,size:[0.15,1.70,0.08],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0.035,0.115,0.9,1.7,0.02,0.08); b.phys(0,0.15,1.06,1.14,0,0.02); b.phys(0.055,0.095,1.58,1.62,0,0.02); b.phys(0.065,0.085,1.38,1.62,-0.01,0); b(0.035,0.115,0.90,1.70,0.02,0.08,mat.body); b(0,0.15,1.06,1.14,0,0.02,mat.lamp); b(0.055,0.095,1.58,1.62,0,0.02,mat.lamp); b(0.065,0.085,1.38,1.62,-0.01,0.0,mat.lamp); }}, // pipe cover, thermostat bar, holder, handset
    {id:'niche8',type:'накладная полка-ниша 0.60×0.10×0.30 на южной стене, открыта на север, LED по верхней кромке',room:8,layer:'bath2',pos:[8.30,13.024],rot:0,size:[0.60,1.35,0.10],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,0.6,1.05,1.07,0,0.1); b.phys(0,0.6,1.33,1.35,0,0.1); b.phys(0,0.02,1.07,1.33,0,0.1); b.phys(0.58,0.6,1.07,1.33,0,0.1); b.phys(0.02,0.58,1.07,1.33,0.08,0.1); b.phys(0.02,0.58,1.32,1.33,0.005,0.02); b(0,0.6,1.05,1.07,0,0.10,mat.body); b(0,0.6,1.33,1.35,0,0.10,mat.body); b(0,0.02,1.07,1.33,0,0.10,mat.body); b(0.58,0.6,1.07,1.33,0,0.10,mat.body); b(0.02,0.58,1.07,1.33,0.08,0.10,mat.body); // bottom, top, sides, back
    ...
    {id:'cove8',type:'скрытый LED-карниз по потолку: северная стена 1.62 и восточная до двери (z 11.59–12.15), h 2.62',room:8,layer:'bath2',pos:[8.235,11.588],rot:0,size:[1.637,2.65,0.562],fixed:'wall',
     build(b){ /* proxy = pre-detail AABBs (realism-all E) */ b.phys(0,1.617,2.55,2.62,0.02,0.06); b.phys(0,1.617,2.62,2.65,0,0.02); b.phys(1.577,1.617,2.55,2.62,0.06,0.562); b.phys(1.617,1.637,2.62,2.65,0.06,0.562); b(0,1.617,2.55,2.62,0.02,0.06,mat.body); b(0,1.617,2.62,2.65,0,0.02,mat.led);                            // north: shadow profile lip and the strip above it
    ...
    {id:'fan8',type:'вентилятор вытяжки Ø0.12 в потолке над унитазом',room:8,layer:'bath2',pos:[9.46,11.69],rot:0,size:[0.12,2.70,0.12],fixed:'wall',
     build(b,g){ b.phys(0,0.12,2.68,2.7,0,0.12); g.add(new THREE.Mesh(new THREE.TorusGeometry(0.055,0.005,8,24).rotateX(Math.PI/2).translate(0.06,2.695,0.06),mat.plastic)); g.add(new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,0.004,24).translate(0.06,2.694,0.06),mat.dark)); [-0.03,-0.01,0.01,0.03].forEach(dx=>b(0.06+dx-0.003,0.06+dx+0.003,2.686,2.692,0.02,0.10,mat.plastic)); }}, // rim, dark intake, four grille bars
    // electrics: flat boxes 0.08 × 0.08 × 0.01
    {id:'sock24',type:'скрытый вывод для полотенцесушителя, h 0.45',room:8,layer:'bath2',pos:[9.51,13.114],rot:0,size:[0.08,0.49,0.01],fixed:'wall',build(b){ b.plate(0,0.08,0.41,0.49,0,0.01,mat.plastic); }},
    ...
    {id:'wpeg',type:'перфопанель 0.60×0.80 для ручного инструмента на восточной стене у торца, вынос крючков ≤ 0.08',room:6,layer:'wardrobe',pos:[6.865,2.30],rot:0,size:[0.02,1.80,0.60],fixed:'wall',
     build(b){ b.phys(0,0.02,1,1.8,0,0.6); b(0,0.02,1.00,1.80,0,0.60,mat.wpanel); }},
    {id:'wmirror',type:'зеркало ростовое 0.50×1.60 без рамы напротив длинной штанги',room:6,layer:'wardrobe',pos:[6.865,3.00],rot:0,size:[0.02,1.90,0.50],fixed:'wall',
     build(b){ b.phys(0,0.02,0.3,1.9,0,0.5); b(0,0.02,0.30,1.90,0,0.50,mat.glass); }},
    {id:'wboard',type:'держатель гладильной доски: две скобы на внутренней стороне двери (доска 1.20×0.35 висит 0.50–1.70)',room:6,layer:'wardrobe',pos:[6.30,3.874],rot:0,size:[0.40,1.20,0.02],fixed:'wall',
     build(b){ [[0,0.04,1.1,1.2,0,0.02],[0.36,0.4,1.1,1.2,0,0.02]].forEach(q=>b.phys(...q)); // proxy = today's AABBs (realism-all F1)
       [0,0.36].forEach(x=>b(x,x+0.04,1.10,1.20,0,0.02,mat.handle)); }},
    ...
    {id:'wstep',type:'складная стремянка 2 ступени на крючке у боковины секции B, низ 0.10 (разложенная 0.40×0.30×0.45)',room:6,layer:'wardrobe',pos:[5.60,2.794],rot:0,size:[0.40,0.58,0.12],fixed:'wall',
     build(b){ [[0,0.4,0.1,0.55,0.02,0.12],[0.18,0.22,0.55,0.58,0,0.02]].forEach(q=>b.phys(...q)); // proxy = today's AABBs (realism-all F1)
       b(0,0.40,0.10,0.55,0.02,0.12,mat.hdark); b(0.18,0.22,0.55,0.58,0,0.02,mat.handle); }},
    {id:'wlight',type:'линейный потолочный светильник 1.85×0.04 над проходом, 4000 K, от датчика движения M11',room:6,layer:'wardrobe',pos:[6.48,1.95],rot:0,size:[0.04,2.70,1.85],fixed:'wall',
     build(b){ b.phys(0,0.04,2.68,2.7,0,1.85); b(0,0.04,2.68,2.70,0,1.85,mat.led); }},
    {id:'led6',type:'LED-лента под передней кромкой антресоли, свет вниз на штанги; включается вместе с M9',room:6,layer:'wardrobe',pos:[6.135,1.90],rot:0,size:[0.02,2.00,1.55],fixed:'wall',
     build(b){ b.phys(0,0.02,1.98,2,0,1.55); b(0,0.02,1.98,2.00,0,1.55,mat.led); }},
    ...
    {id:'sock25',type:'розетка у двери для пылесоса и утюга, h 0.30',room:6,layer:'wardrobe',pos:[6.875,3.55],rot:0,size:[0.01,0.34,0.08],fixed:'wall',build(b){ b.phys(0,0.01,0.26,0.34,0,0.08); b(0,0.01,0.26,0.34,0,0.08,mat.lamp); }},
    ...
    {id:'cable10',type:'кабель-канал 0.06×0.04 по западной стене на 2.23–2.27, выше проёма в кухню (2.10): питание и сеть между полкой и стеллажом',room:10,layer:'balcony',pos:[13.91,2.40],rot:0,size:[0.06,2.27,3.00],fixed:'wall',
     build(b){ b.phys(0,0.06,2.23,2.27,0,3); b(0,0.06,2.23,2.27,0,3.00,mat.wpanel); }},
    // electrics: flat boxes 0.08 × 0.08 × 0.01
    {id:'sock26',type:'розеточный блок ИТ над полкой: 6 розеток + ввод Ethernet, отдельная линия, h 2.25',room:10,layer:'balcony',pos:[14.17,6.033],rot:0,size:[0.08,2.29,0.01],fixed:'wall',build(b){ b.phys(0,0.08,2.21,2.29,0,0.01); b(0,0.08,2.21,2.29,0,0.01,mat.lamp); }},
    {id:'sock27',type:'розетки 2+2 USB над столешницей у восточного края, h 0.90',room:10,layer:'balcony',pos:[14.77,6.033],rot:0,size:[0.08,0.94,0.01],fixed:'wall',build(b){ b.phys(0,0.08,0.86,0.94,0,0.01); b(0,0.08,0.86,0.94,0,0.01,mat.lamp); }},
    {id:'sock28',type:'розеточный блок над стеллажом: 4 розетки для площадки 2.05–2.70, h 2.25',room:10,layer:'balcony',pos:[14.17,2.268],rot:0,size:[0.08,2.29,0.01],fixed:'wall',build(b){ b.phys(0,0.08,2.21,2.29,0,0.01); b(0,0.08,2.21,2.29,0,0.01,mat.lamp); }},
    {id:'sock29',type:'розетка в открытой секции стеллажа 0.75–1.075 (зарядки), h 1.00',room:10,layer:'balcony',pos:[14.77,2.268],rot:0,size:[0.08,1.04,0.01],fixed:'wall',build(b){ b.phys(0,0.08,0.96,1.04,0,0.01); b(0,0.08,0.96,1.04,0,0.01,mat.lamp); }},
    {id:'led7',type:'LED-лента под передней кромкой ИТ-полки, свет на столешницу, 4000 K; выключатель на торце полки',room:10,layer:'balcony',pos:[13.95,5.44],rot:0,size:[1.11,2.00,0.02],fixed:'wall',
     build(b){ b.phys(0,1.11,1.98,2,0,0.02); b(0,1.11,1.98,2.00,0,0.02,mat.led); }},
    {id:'blight',type:'линейный потолочный светильник 2.20×0.04 по оси лоджии от проёма до стула, 4000 K',room:10,layer:'balcony',pos:[14.48,2.90],rot:0,size:[0.04,2.70,2.20],fixed:'wall',
     build(b){ b.phys(0,0.04,2.68,2.7,0,2.2); b(0,0.04,2.68,2.70,0,2.20,mat.led); }},
    {id:'sw8',type:'выключатель потолочного света на западной стене южнее проёма (проём до 4.971), h 0.95',room:10,layer:'balcony',pos:[13.91,5.02],rot:0,size:[0.01,0.99,0.08],fixed:'wall',build(b){ b.phys(0,0.01,0.91,0.99,0,0.08); b(0,0.01,0.91,0.99,0,0.08,mat.lamp); }},
    {id:'blinds10',type:'рулонные солнцезащитные шторы: кассеты по верху остекления z 2.40–5.90 (собраны)',room:10,layer:'balcony',pos:[15.02,2.40],rot:0,size:[0.08,2.30,3.50],fixed:'wall',
     build(b){ b.phys(0,0.08,2.22,2.3,0,3.5); b(0,0.08,2.22,2.30,0,3.50,mat.wpanel); }},
  ];
