// Light schemes and the image pipeline of the 3D view (materials-lighting M0, L1). The daylight scheme is a fixed
// overcast sky: a cool hemisphere plus a weak shadowless directional light low over the window side, nothing follows
// the camera; it exists to judge materials, not to depict the apartment's light. The lamps scheme (L1) is the apartment's own fixtures: LIGHTS lists every source
...
const LIGHTING={scheme:'lamps',lit:false,exposure:0.75,schemes:['neutral','lamps'],groups:{},lights:[]};
window.LIGHTING=LIGHTING;
const hemi=new THREE.HemisphereLight(0xffffff,0xa8a49c,1.0); scene.add(hemi);
const sun=new THREE.DirectionalLight(0xffffff,0.55); sun.position.set(-10,6,-7); scene.add(sun); // low over the window side: light that reads as coming in, not down through the roof
...
(function(){
  ...
  // daylight scheme: overcast sky — the cool hemisphere carries the light, the weak sun only separates walls by their
  // facing and casts nothing (the ceiling does not block it, so its shadows read as light through the roof).
  // lamps scheme: no sun, an explicit weak fill standing in for bounced light, and the LIGHTS catalogue
  const NEUTRAL={lit:{sun:0.25,hemi:0.4},flat:{sun:0.55,hemi:1.0},sky:0xd9e3f0,ground:0xb0aca4}, LAMPS={sun:0,hemi:0.15,sky:0xffffff,ground:0xa8a49c};
  ...
  function environment(scheme){ // procedural room for reflections (mirror, chrome, glass) and ambient IBL, prefiltered once per scheme:
    scheme=(scheme||LIGHTING.scheme)==='neutral'?'neutral':'lamps'; if(envs[scheme]) return envs[scheme]; // neutral — cool grey box, plain ceiling, one wide diffuse window (overcast day); lamps — dim warm box without a window (bounced lamp light, not daylight)
    const lamps=scheme==='lamps', s=new THREE.Scene(), Bm=c=>new THREE.MeshBasicMaterial({color:c,side:THREE.BackSide});
    s.add(new THREE.Mesh(new THREE.BoxGeometry(8,3,8),Bm(lamps?0x2a2826:0x6e7176))); const ceil=new THREE.Mesh(new THREE.PlaneGeometry(8,8),new THREE.MeshBasicMaterial({color:lamps?0x4a4640:0x8d9096})); ceil.rotation.x=Math.PI/2; ceil.position.y=1.49; s.add(ceil);
    if(!lamps){ const win=new THREE.Mesh(new THREE.PlaneGeometry(3.6,2.2),new THREE.MeshBasicMaterial({color:0xdce4ee})); win.position.set(0,0.1,-3.99); s.add(win); } // wide and only slightly brighter than the walls: overcast light has no hot spot
  ...
  const NEAR=4, POOL={spot:16,point:6}; let SHADOWS=1; // shadow maps per frame (L3 p.3): one candidate of the camera's room is enough; 0 switches them off // neighbour reach in metres; constant visible counts per type (padding at intensity 0) so programs are compiled once
  ...
  LIGHTING.apply=function(lit){ // lit = 3D view with the fixed pipeline; false = plan view (flat, linear, no shadows)
    LIGHTING.lit=lit=!!lit; const lamps=lit&&LIGHTING.scheme==='lamps', n=!lit?NEUTRAL.flat:lamps?LAMPS:NEUTRAL.lit, c=lamps?LAMPS:NEUTRAL;
    sun.intensity=n.sun; hemi.intensity=n.hemi; hemi.color.set(c.sky); hemi.groundColor.set(c.ground);
    ...
    renderer.setPixelRatio(Math.min(devicePixelRatio,lamps?1.25:2)); // L3 p.4: Retina at 2 is 4x the fragments for every lamp
  ...
  LIGHTING.set=function(scheme){ // user choice, remembered; re-applies the current view state
    if(!LIGHTING.schemes.includes(scheme)) scheme='lamps';
  ...
  LIGHTING.set(saved||'lamps'); // внутренний B по умолчанию, выбор пользователя из localStorage сохраняется
})();
