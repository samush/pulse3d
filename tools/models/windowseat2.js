// Window seat 0.75×0.77×1.702 in room 2 (tasks/realism-all/PLAN.md §9): body with a recessed plinth, a radiator grille west (x=0), mattress r 30 with a dome, two pillows at the towers. Material names: paint/chrome slots, kmat (mattress), pillow. Pivot NW corner, back at the east wall, metres.
...
add(new THREE.BoxGeometry(0.68,0.05,L-0.10).translate(0.39,0.025,L/2),'paint');        // plinth, recessed 0.05 from the front
add(rbox(0.73,0.52,L,0.005,0.02,0.05,0,{m:1,step:0.3}),'paint');                       // body 0.05–0.57: top over the sill (0.56), so the mattress lies on it (user, 2026-09-13)
grille(0.01,0.06,0.44,0.01,L-0.01,'paint');                                            // front grille west instead of drawers: air from the radiator under the sill
add(rbox(0.73,0.08,L-0.04,0.03,0.02,0.57,0.02,{m:2,step:0.1,crown:0.01}),'kmat');      // mattress 0.57–0.65
[0.05,L-0.35].forEach(z=>add(rbox(0.40,0.11,0.30,0.05,0.185,0.65,z,{m:2,step:0.1,crown:0.01}),'pillow')); // pillows, top ≤ 0.77
