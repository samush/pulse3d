...
const outline=(inset,xw=inset)=>{ const p=new THREE.Path(); const z0=inset, z1=L-inset; p.moveTo(xw,-z0); for(let i=0;i<=N;i++){ const z=z0+(z1-z0)*i/N; p.lineTo(wo(z)-inset,-z); } p.lineTo(xw,-z1); p.closePath(); return p; }; // shape y = -z, see rotateX below
const parts=[], add=(geo,mat)=>parts.push({geo,mat});
const shell=new THREE.Shape(); shell.curves=outline(0).curves; shell.holes.push(new THREE.Path(outline(W,W+0.01).getPoints().reverse())); // hole wound opposite to the shell: same winding leaves the inner walls back-facing (culled, tile shows through); wall-side face 1 cm off the tile plane
...
const floor=new THREE.Shape(); floor.curves=outline(W-0.002,W+0.008).curves;
