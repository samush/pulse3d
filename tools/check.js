...
(async () => {
  ...
  const fitD = await page.evaluate(() => { const fit = id => { const g = ITEM_GROUPS[id], bb = new THREE.Box3().setFromObject(g), inv = new THREE.Matrix4().copy(g.matrixWorld).invert(); bb.applyMatrix4(inv); const s = g.userData.size; return bb.min.x >= -0.001 && bb.min.y >= -0.001 && bb.min.z >= -0.001 && bb.max.x <= s[0] + 0.001 && bb.max.y <= s[1] + 0.001 && bb.max.z <= s[2] + 0.001; };
    const one = ['sw3', 'sw4', 'sw6', 'sock14', 'sock15', 'sock16', 'sock17', 'sock18', 'sock19', 'sock20', 'led4', 'led5'], ids = ['mlight', 'bra5', 'bra6', 'bra7', 'bra8'].concat(one);
  ...
  const stageBGlb = await page.evaluate(async () => { const ids = ['kidsofa', 'kidchair', 'kidchair2', 'windowseat1']; for (let i = 0; i < 100 && !ids.every(id => ITEM_GROUPS[id].userData.glbLoaded || (VIZ.loadErrors || []).some(s => s.startsWith(id + ':'))); i++) await new Promise(r => setTimeout(r, 100));
  ...
  stageBGlb.forEach(r => { if (!r.loaded || r.warn || !r.grey || !r.fits || r.boxes !== { kidsofa: 8, kidchair: 5, kidchair2: 5, windowseat1: 9 }[r.id]) problems.push('glb: ' + r.id + ' — загрузка/валидация/серый/габарит/proxy не сошлись (realism-all §8): ' + JSON.stringify(r)); });
