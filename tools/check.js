...
(async () => {
  ...
  await page.evaluate(() => { try { localStorage.setItem('pulse3d.bed1', 'original'); localStorage.setItem('pulse3d.bed2', 'original'); localStorage.setItem('pulse3d.desk2', 'A'); localStorage.setItem('pulse3d.k4.table', 'A'); localStorage.setItem('pulse3d.m3.vtable', 'A'); localStorage.setItem('pulse3d.b9.tub', 'A'); } catch (e) {} KIDBED.set('original'); DESK2.set('A'); ROOM4.set('table', 'A'); ROOM3.set('vtable', 'A'); BATH9.set('tub', 'A'); }); // the room checks below describe the original beds and the room 2 desk; the page defaults to F / B since 2026-09-09, and the keys keep that across the reloads below
  ...
  else if (wsGlb.warn || wsGlb.size !== '0.75,0.77,1.7' || wsGlb.slots !== 'cabinetPaint,fabric' || wsGlb.boxes !== 6) problems.push('glb: windowseat2 — предупреждения/габарит/слоты/proxy не сошлись: ' + JSON.stringify(wsGlb));
  ...
  const proxD = await page.evaluate(() => { const ext = id => { const bb = new THREE.Box3(); PHYS[id].forEach(m => bb.union(new THREE.Box3().setFromObject(m))); const s = new THREE.Vector3(); bb.getSize(s); return [s.x, s.y, s.z].map(v => Math.round(v * 1000) / 1000).join(); };
    const want = { mbed: [6, '2.015,1.1,3.337'], mcab: [19, '2.015,0.85,0.365'], mward: [19, '1.4,2.7,0.59'], mtv: [2, '0.97,0.56,0.04'], vanity: [6, '1.3,0.37,0.42'], vmirror: [3, '1.14,1.14,0.026'], vpouf: [5, '0.4,0.45,0.4'], mrug: [1, '2,0.01,2'], mcurtain: [3, '0.06,2.66,3.017'], bra5: [3, '0.12,0.12,0.225'], bra6: [3, '0.12,0.12,0.225'] };
    return Object.entries(want).filter(([id, [n, e]]) => PHYS[id].length !== n || ext(id) !== e || !ITEM_GROUPS[id].userData.proxy.length).map(([id]) => id + ':' + PHYS[id].length + ':' + ext(id)); });
  ...
  const fitE = await page.evaluate(() => ['basin', 'basindrawer', 'basinmixer', 'tubmixer', 'shower', 'bathmirror', 'towelrail', 'towel8', 'wcbox', 'wcbox8', 'mirror8', 'curb8e', 'drain8', 'glass8', 'cove8', 'fan', 'fan8', 'rain8', 'sock21', 'sock22', 'sock24'].filter(id => { const g = ITEM_GROUPS[id], bb = new THREE.Box3().setFromObject(g).applyMatrix4(new THREE.Matrix4().copy(g.matrixWorld).invert()), s = g.userData.size; return !(bb.min.x >= -0.001 && bb.min.y >= -0.001 && bb.min.z >= -0.001 && bb.max.x <= s[0] + 0.001 && bb.max.y <= s[1] + 0.001 && bb.max.z <= s[2] + 0.001); }));
  ...
  const stageB = await page.evaluate(() => { const want = { kidbed: 9, kiddesk: 2, kidped: 4, kidchair: 5, kidshelf: 13, kidshelf2: 8, kidshelf3: 3, windowseat1: 5, kidsofa: 8, kidrug: 1, projector: 3, screen: 2, curtain: 2, kidlight: 1, track: 3, bra1: 3, bra2: 2, sw1: 1, sock1: 1, sock2: 1, sock3: 1, sock4: 1, sock5: 1, sock6: 1, sock7: 1 };
  ...
  stageBGlb.forEach(r => { if (!r.loaded || r.warn || !r.grey || !r.fits || r.boxes !== { kidsofa: 8, kidchair: 5, kidchair2: 5, windowseat1: 5, bchair: 6 }[r.id]) problems.push('glb: ' + r.id + ' — загрузка/валидация/серый/габарит/proxy не сошлись (realism-all §8): ' + JSON.stringify(r)); });
  ...
  const M4 = { hall5: { items: [['wardrobe', 'door', 'whiteOakFront'], ['wardrobe', 'body', 'whiteOakFront'], ['wardrobe', 'table', 'oakFloor'], ['entry', 'door', 'cabinetPaint'], ['sw5', 'plastic', 'plastic'], ['pouf', 'glb:leather', 'class']], pose: [9.1, 6.6, 9.1, 7.7] },
    ...
    bath8: { items: [['curb8e', 'top', 'tile6060'], ['mirror8', 'mirror', 'class'], ['wcbox8', 'plastic', 'plastic'], ['cove8', 'body', 'class'], ['glass8', 'glass', 'class'], ['wc8', 'glb:ceramic', 'class'], ['drain8', 'handle', 'class'], ['fan8', 'plastic', 'plastic']], pose: [9.6, 12.75, 8.75, 12.2] },
  };
  ...
  if (mir.ids.join() !== 'bathmirror,lmirror,mirror,vmirror,wmirror') problems.push('зеркала: слот mirror у ' + mir.ids.join(',') + ' (mirrors.md: bathmirror, lmirror, mirror, vmirror, wmirror)');
  ...
  const l1 = await page.evaluate(() => {
    ...
    LIGHTING.set('neutral'); out.neutral = L.every(l => l.intensity === 0 && !l.castShadow) && sun.intensity > 0 && !sun.castShadow && led('g4.sofa').every(m => m.emissiveIntensity === 1); LIGHTING.set('lamps'); // overcast daylight: a weak sun, no shadows through the ceiling
  ...
  // kids' loft bed variants A/B (tasks/kids-loft-beds): rebuilt in place with the same pose and nine PHYS boxes, stairs S1 = seven equal rises to 1.80 (30 mm treads,
  // the seventh a 0.24 landing), nothing outside the item size; 20 switches leave no stray meshes; back to original before the later checks
  const loft = await page.evaluate(() => {
    ...
    for (const [v, n] of [['ladder', 0], ['gentle', 8], ['loft', 0]]) { KIDBED.set(v); for (const id of ['kidbed', 'kidbed2']) { const g = ITEM_GROUPS[id], N = v === 'gentle' ? (id === 'kidbed' ? 8 : 7) : 0;
    ...
    for (const v of ['timber', 'steel']) { KIDBED.set(v); for (const id of ['kidbed', 'kidbed2']) { const g = ITEM_GROUPS[id], u = g.userData, box = bb(g), p = u.pos;
      ...
      if (!g.children.some(o => o.isLight)) out.bad.push(v + ' ' + id + ': светильник кровати потерян'); } }
    for (let i = 0; i < 5; i++) { KIDBED.set('timber'); KIDBED.set('steel'); KIDBED.set('ladder'); KIDBED.set('gentle'); KIDBED.set('loft'); } KIDBED.set('original'); out.n1 = count(); out.variant = KIDBED.variant;
    return out; });
  ...
  if (Math.abs(m3.bedEast - 14.76) > 0.01) problems.push('комната 3: кровать не вплотную к стене у окна (x 14.76): x1=' + m3.bedEast.toFixed(3));
