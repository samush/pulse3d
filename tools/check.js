...
(async () => {
  ...
  const kid = await page.evaluate(() => {
    ...
    const deskStraight = desk.max.z - desk.min.z <= 0.61 && desk.max.z > 4.86 && desk.max.x - desk.min.x > 1.9;
    ...
    return { n: kids.length, inside, hitPlat, warn, tops, sofaTop, bedPos: ITEM_GROUPS.kidbed.userData.pos, colored, deskStraight, seatOk, windowFree };
  });
